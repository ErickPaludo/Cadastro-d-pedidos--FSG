...
namespace Pedidos
{
    public class ControllerPedido
    {
        ...
        public void Cadastrar()
        {
            if (id_select != 0)
            {
                List<ItensPedidos> itemPedido = new List<ItensPedidos>();
                ...
                if (qtn_linhas > 0)
                {
                    ...
                    {
                        itemPedido.Add(new ItensPedidos
                        {
                            Id = item.Ditens.FirstOrDefault(x => x.Key == Convert.ToInt32(viewpedido.ItensSelecionados.Rows[i].Cells[0].Value)).Key,
                            Quantidade = Convert.ToInt32(viewpedido.ItensSelecionados.Rows[i].Cells[3].Value)
                        });
                ...
                if (edit == false)
                {
                    model.AddPedido(new Pedidos(id_select,itemPedido,itemPedido.Sum(x=>x.Quantidade)));
                    Atualiza();
...
public void ItensDoPedido(int id_pedido)
{
    ...
    foreach (var obj in pedidos.Dpedido)
    {
        ...
        {
            for (int i = 1; i <= obj.Value.Itens.Count; i++)
            {

                
                    Item dados = item.Ditens[slotValue];
                    viewpedido.TabelaPitens.Rows.Add(i, dados.Nome_item, slotQuant, dados.Preco_item);
